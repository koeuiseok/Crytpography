...
void asm_hight_enc_table(unsigned char* text, unsigned char* use_key, unsigned char* HIGHT_F0, unsigned char* HIGHT_F1 )
{
  // 초기 변환(wk와 연산)
...
int main(void)
{
    ...
    asm_hight_enc_table(plaintext, use_key,HIGHT_F0, HIGHT_F1);
    
    unsigned char use_key2[136] = { 0, };						
    unsigned char masterkey2[16] = {  0xff ,0xee ,0xdd, 0xcc , 0xbb, 0xaa , 0x99 , 0x88 ,0x77 ,0x66 ,0x55 , 0x44 , 0x33 ,0x22, 0x11, 0x00}; 		
    unsigned char plaintext2[8] = { 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66 , 0x77  }; 
    
    HIGHT_keygen(masterkey2, use_key2 );
    
    asm_hight_enc_table(plaintext2, use_key2,HIGHT_F0, HIGHT_F1);
