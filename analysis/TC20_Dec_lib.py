#-------------------------
# 암호분석 과제2 TC20 DEC
#--------------------------
import os
os.chdir('../')
os.chdir(os.path.dirname(os.path.abspath(__file__)))

Sbox = [
        
        ]

def AR(in_state, rkey):
    out_state = [0, 0, 0, 0]
    for i in range(len(in_state)):
        out_state[i] = in_state[i] ^ rkey[i]
    return out_state

def InvSB(in_state):
    output=[]
    for i in range(0,256):
        output.append(i)
    
    for j in range(len(output)):
        output[Sbox[j]] = j   
        
    for h in range(len(in_state)):
        in_state[h] = output[in_state[h]]
    return in_state

def inv_LM(in_state):
    out_state = [0, 0, 0, 0]
    all_xor = in_state[0] ^ in_state[1] ^ in_state[2] ^ in_state[3]
    for i in range(len(in_state)):
        out_state[i] = in_state[i] ^ all_xor
    return out_state    

def Dec_Round(in_state, rkey):
    out_state1 = [0, 0, 0, 0]
    out_state2 = [0, 0, 0, 0]
    out_state3 = [0, 0, 0, 0]
    out_state1 = inv_LM(in_state)
    out_state2 = InvSB(out_state1)
    out_state3 = AR(out_state2, rkey)
    
    return out_state3

def TC20_Dec(input_state, key):
    state = input_state
    numRound = 10 # 라운드 수
    for i in range(0, numRound):
        state = Dec_Round(state, key)

    return state

def main():
    key = [0, 1, 2, 3]
    input_state =[246, 31, 30, 133]
    
    output_state = [ item for item in TC20_Dec(input_state, key)  ]
    msg = [chr(ch) for ch in output_state]
    print('input ciphertext =', input_state)
    print('output plaintext =', output_state)
    print('plain message = ', msg)

##-- Run main()

if __name__ == '__main__':
    main()
    

